...
public class SMVWIG {
	...
	public double computeSMVWIG(double[] scores){
		...
		for(int i=0;i<k;i++){
			sum=sum+Math.abs(Math.log(scores[i]/u))*(scores[i]-scoreD);
		}
		numerator=sum/k;
	...
	/**
	 * 根据input.runId计算每个query的SMVWIG值, 并将SMVWIGScore存入文件 
	 *  
	 * */
	public void getSMVWIGScores(String input,String output){
		...
		double SMVWIGScore=0;//临时存放一个query的SMVWIG值
		...
		try{
			...
			while((tempLine=lineNumberReader.readLine())!=null){
				...
				//queryId不同,计算preQueryId的SMVWIGScore,写入文件,清空arrayList信息,处理terms信息
				if(!preQueryId.equalsIgnoreCase(terms[0])){
					...
					//调用computSMVWIG()计算此query的SMVWIG值
					SMVWIGScore=computeSMVWIG(scores);
					//把queryId和SMVWIGScore写入文件
					fileWriter.write("queryId:\t"+preQueryId+"\tSMVWIG:\t"+SMVWIGScore+"\n");
					//清空arrayList
					arrayList.clear();
					//开始处理terms信息
					preQueryId=terms[0];
			...
			//最后queryId对应的scores未处理,计算其SMVWIGScore,并写入文件
			//把arrayList转化为double数组
			scoreCount=arrayList.size();
			scores=new double[scoreCount];
			for(int i=0;i<scoreCount;i++)
