...
public class NQCWIG_2 {
	...
	/**
	 * 根据input.runId计算每个query的NQCWIG_2值, 并将NQCWIG_2Score存入文件 
	 *  
	 * */
	public void getNQCWIG_2Scores(String input,String output){
		...
		double NQCWIG_2Score=0;//临时存放一个query的NQCWIG_2值
		...
		try{
			...
			while((tempLine=lineNumberReader.readLine())!=null){
				...
				//queryId不同,计算preQueryId的NQCWIG_2Score,写入文件,清空arrayList信息,处理terms信息
				if(!preQueryId.equalsIgnoreCase(terms[0])){
					...
					//调用computNQCWIG_2()计算此query的NQCWIG_2值
					NQCWIG_2Score=computeNQCWIG_2(scores);
					//把queryId和NQCWIG_2Score写入文件
			...
			//最后queryId对应的scores未处理,计算其NQCWIG_2Score,并写入文件
			//把arrayList转化为double数组
			scoreCount=arrayList.size();
			scores=new double[scoreCount];
			for(int i=0;i<scoreCount;i++)
