...
public class NQCSMV {
	...
	public double computeNQCSMV(double[] scores){
		...
		for(int i=0;i<k;i++){
			sum=sum+(scores[i]-u)*(scores[i]-u)*scores[i]*scores[i];
		}
	...
	/**
	 * 根据input.runId计算每个query的NQCSMV值, 并将NQCSMVScore存入文件 
	 *  
	 * */
	public void getNQCSMVScores(String input,String output){
		...
		double NQCSMVScore=0;//临时存放一个query的NQCSMV值
		...
		try{
			...
			while((tempLine=lineNumberReader.readLine())!=null){
				...
				//queryId不同,计算preQueryId的NQCSMVScore,写入文件,清空arrayList信息,处理terms信息
				if(!preQueryId.equalsIgnoreCase(terms[0])){
					...
					//调用computNQCSMV()计算此query的NQCSMV值
					NQCSMVScore=computeNQCSMV(scores);
					//把queryId和NQCSMVScore写入文件
					fileWriter.write("queryId:\t"+preQueryId+"\tNQCSMV:\t"+NQCSMVScore+"\n");
					//清空arrayList
					arrayList.clear();
					//开始处理terms信息
					preQueryId=terms[0];
			...
			//最后queryId对应的scores未处理,计算其NQCSMVScore,并写入文件
			//把arrayList转化为double数组
			scoreCount=arrayList.size();
			scores=new double[scoreCount];
			for(int i=0;i<scoreCount;i++)
